...
@Composable
fun EditMealScreen(vm: MealsViewModel, entryId: Int, onCompleted: () -> Unit, onBack: () -> Unit) {
    val entry = vm.entries.find { it.id == entryId } ?: return

    var grams by remember { mutableStateOf(entry.grams) }
    var section by remember { mutableStateOf(entry.section) }

    FormScaffold(title = "Editar entrada", onBack = onBack) {
        SelectedItemHeader(Icons.Default.Restaurant, entry.meal.name, "${entry.meal.calories} cal / 100g")
        ...
        Button(
            ...
            onClick = { vm.updateEntry(entryId, grams, section); onCompleted() },
            shape = RoundedCornerShape(16.dp),
